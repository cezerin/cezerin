...
const SortableItem = SortableElement(({ image, onDelete, onImageEdit }) =>
	<li className={style.item}><GalleryItem url={image.url} alt={image.alt} id={image.id} onDelete={onDelete} onImageEdit={() => { onImageEdit(image) }} /></li>
)

const SortableList = SortableContainer(({items, onDelete, onImageEdit}) =>
		<ul className={style.list}>
			{items.map((value, index) =>
        <SortableItem key={`item-${index}`} index={index} image={value} onDelete={onDelete} onImageEdit={onImageEdit} />
      )}
		</ul>
)

const Gallery = ({ productId, images, onImageDelete, onImageSort, onImageUpload, uploading, onImageEdit }) => {

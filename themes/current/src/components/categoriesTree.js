...
const TreeElement = ({ categories, currentCategory, activeCategory }) => {
  ...
  return (
    <li className={currentCategoryIsActive || parentCategoryIsActive ? 'active' : ''}>
      <Link to={currentCategory.path} activeClassName="active">{currentCategory.name}</Link>
...
const Tree = ({ categories, activeCategory }) => {
  if(categories) {
    const elements = categories.filter(category => category.parent_id === null).map((category, index) => <TreeElement key={index} categories={categories} currentCategory={category} activeCategory={activeCategory} />)
    return (

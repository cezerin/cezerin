...
const GalleryItemImage = ({ category }) => {
	const imageUrl =
		category.image && category.image.length > 0
			? helper.getThumbnailUrl(
					category.image,
					themeSettings.category_list_thumbnail_width
			  )
			: '';
...
const CategoryGallery = ({ categories }) => {
	const items = categories
		...
		.map((category, index) => <GalleryItem key={index} category={category} />);
	return <div className="columns is-multiline is-mobile">{items}</div>;
};
